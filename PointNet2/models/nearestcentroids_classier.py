...
class NCC_LVQ21(nn.Module):
    def __init__(self, device, num_classes, k=15, mu=0.999, mode = 'train'):
        ...
        self.n_updates = 0
        self.step = 1e-4
        self.n_updates_to_stepdrop = 984 * 200 + 10
        self.minstep = 1e-6
        self.epsilon = 0.4
        self.slowdown_rate = 0.4

        self.pixel_update_freq = 10
        self.pixel_size = self.K * 20

        self.register_buffer("point_queue", torch.randn((self.num_classes, self.pixel_size, self.dim),requires_grad=False).to(self.dev))
