...
from models.nearestcentroids_classier import NCC_LVQ21
...
def parse_args():
    ...
    parser.add_argument('--log_dir', type=str, default='pointnet2_cls_msg_ip3d', help='experiment root')
    parser.add_argument('--decay_rate', type=float, default=1e-4, help='decay rate')
    parser.add_argument('--use_normals', default=True, help='use normals')
    parser.add_argument('--process_data', default=True, help='save data offline')
    parser.add_argument('--use_uniform_sample', default=True, help='use uniform sampiling')
...
def main(args):
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        with torch.no_grad():
            instance_acc, class_acc = test(classifier.eval(), classier, testDataLoader, num_class=num_class)
...
def predict(cosine_sim, predict_mode="max"):
    logits = torch.max(cosine_sim, dim=2)[0]
    preds = torch.argmax(logits, dim=1)
    return preds


def test(model, classier, loader, num_class=40):
    mean_correct = []
